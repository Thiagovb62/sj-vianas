...
if (!process.env.DATABASE_URL) {
  ...
  console.warn('O banco de dados do Supabase não será acessível.');
}

// Configuração do pool de conexões com o Supabase (PostgreSQL)
const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: process.env.DATABASE_URL && (process.env.DATABASE_URL.includes('localhost') || process.env.DATABASE_URL.includes('127.0.0.1'))
    ? false
    : { rejectUnauthorized: false } // SSL obrigatório para o Supabase na nuvem
});
...
app.get('/api/data', async (req, res) => {
  try {
    ...
    // Formata o retorno para manter compatibilidade com as chaves CamelCase do frontend
    ...
    console.error('Erro ao buscar dados do Postgres:', err);
...
app.post('/api/people', async (req, res) => {
  ...
  try {
    ...
    const status = isChild ? null : 'pendente'; // Se for criança não tem status (isento)
...
// Inicialização e conexão do banco antes de abrir o servidor
initDb()
  .then(() => {
    app.listen(PORT, () => {
